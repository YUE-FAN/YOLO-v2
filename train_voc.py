...
# TODO: try out lr schedule in https://github.com/TencentYoutuResearch/ObjectDetection-OneStageDet/issues/36
#     lr_steps: [400,700,900,1000, 40000,60000]
#     lr_rates: [0.0005,0.001,0.002,0.02, 0.002,0.0002]
#     warmup_lr: 0.0001
#     max_batches: 80200, mini_batch_size: 16, batch_size: 64, input_shape: [608, 608]
def get_args():
    ...
    parser.add_argument("--pre_trained_model_type", type=str, choices=["model", "params", "scratch"], default="params")
    parser.add_argument("--pre_trained_model_path", type=str, default="trained_models/only_params_trained_yolo_voc")#whole_model_trained_yolo_voc
...
def train(opt):
    ...
    if torch.cuda.is_available():
        # writer.add_graph(model.cpu(), torch.rand(opt.batch_size, 3, opt.image_size, opt.image_size)) #TODO:FUCK the writer
        model.cuda()
    else:
    ...
    for epoch in range(opt.num_epoches):
        ...
        for iter, batch in enumerate(training_generator):
            global_lr = adjust_learning_rate(opt, optimizer, epoch, iter, len(training_generator))
            image, label = batch
            if torch.cuda.is_available():
            ...
            writer.add_scalar('Train/lr', global_lr, epoch * num_iter_per_epoch + iter)
...
def adjust_learning_rate(opt, optimizer, epoch, iteration, num_iter):
    ...
    max_iter = opt.num_epoches * num_iter
    global_lr = opt.lr * (1 + cos(pi * current_iter / max_iter)) / 2
    for param_group in optimizer.param_groups:
