...
def post_processing(logits, image_size, gt_classes, anchors, conf_threshold, nms_threshold):
    ...
    logits[:, :, 4, :].sigmoid_()  # objectness score

    with torch.no_grad():
        cls_scores = torch.nn.functional.softmax(logits[:, :, 5:, :], 2)  # cls probs
    cls_max, cls_max_idx = torch.max(cls_scores, 2)  # [bs, num_anchors, h*w]
    ...
    if score_thresh.sum() == 0:
        ...
        scores = cls_max[score_thresh]  # probs
        idx = cls_max_idx[score_thresh]  # index of class with max prob
